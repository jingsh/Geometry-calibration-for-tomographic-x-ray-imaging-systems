...
%% Check the number of features, if less than 6, throw an error.
if size(feature3D,1) < 6
...
%% Nomalize 3D features, using standard way to normalize the data to preserve the data accuracy
%%% (x_norm,y_norm,z_norm) =
%%% ((x,y,z)-(x_mean,y_mean,z_mean))/sqrt(sigma_x^2+sigma_y^2+sigma_z^2)
...
%% Normalize 2D features, using the same method.
...
%%% Reference to the paper for detail.
...
%% data denomalization
%%% T4 is the reverse operation of the 3D normalization
%%% T3 is the reverse operation of the 2D normalization
...
%%% P is a 3x4 matrix, where the left 3x3 (Pleft) is the instrinsic matrix
%%% K (after rotation) of the projection. It includes all the parameters of
%%% the imaging system. Check the paper for reference.
Pleft = P(:,1:3);
S = [0,0,1;0,1,0;1,0,0];
PL = Pleft'*S;
[Q,R] = qr(PL,0);
...
%% Translation vector
%%% The 3D translation vector is included in the projection matrix.
Tr = zeros(3,1);
Tr(3) = P(3,4);
...
%% Finalize calculation, extract results from the intrinsic matrix.
Xc = K(1,3);
Yc = K(2,3);
Zc = K(1,1);
end
